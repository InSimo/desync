...
// windowsCacheSizesState holds the file and mapping handles for cleanup.
// Since multiple SizeLimitStores could exist, this is stored per-instance
// via the closeCacheSizesFile function's closure behavior. For simplicity
// we use package-level state (only one cache sizes file per process).
...
// mmap'd byte slice. Uses a real file handle (not INVALID_HANDLE_VALUE) so
// that the mapping is persistent on disk.
func openCacheSizesFile(path string, size int) ([]byte, bool, error) {
	...
	if isNew || info.Size() != int64(size) {
		// Set the file size by seeking and setting end-of-file.
	...
	// Create a file mapping backed by the real file.
	mapping, _, err := procCreateFileMappingW.Call(
		...
		0,
		uintptr(size),
		0, // no name — file-backed, not named mapping
	)
	if mapping == 0 {
		syscall.CloseHandle(handle)
...
// closeCacheSizesFile unmaps the view and closes the mapping and file handles.
...
// syncCacheSizesFile flushes the mmap'd view to disk.
