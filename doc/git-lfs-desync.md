...
| `--max-in-flight`                   | `2G`                                        | Maximum total bytes allowed in-flight across all concurrent agent processes. Accepts human-readable suffixes (e.g. `2G`, `500M`, `1.5GB`; binary units). Limits memory usage when git-lfs spawns multiple agents (`concurrent = true`). Set to `0` to disable. Also settable via `defaults.max-in-flight` in the config or `DESYNC_MAX_INFLIGHT` env var. |
...
| `defaults.max-in-flight` | string         | Maximum total in-flight bytes across all processes, e.g. `"2G"`, `"500M"`, or raw bytes as a string. `0` to disable. Overridden by `--max-in-flight` flag or `DESYNC_MAX_INFLIGHT` env var. |
